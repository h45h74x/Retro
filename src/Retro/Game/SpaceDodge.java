...
import Retro.Entity.Entity;
import Retro.Entity.Surface;
...
public class SpaceDodge extends Game {
    ...
    private void setup() {
        try {
            bg = LookAndFeel.getImage(Const.SpaceImpact.bg);
        ...
        character = new Entity(charactersize, charactersize, Const.SpaceDodge.iconpaths[0]);
    ...
    private void newObstacle() {
        ...
        Surface hole = new Surface(charactersize, holesize);
        hole.setLocation(distance * (maxObstacles), holepos);
        hole.setVelX(xStep);

        Surface obst_top = new Surface(charactersize, holepos);
        obst_top.setLocation(distance * (maxObstacles), 0);
        obst_top.setVelX(xStep);

        Surface obst_bot = new Surface(charactersize, height - (holepos + holesize));
    ...
    private void slow() {
        ...
        if (xCond) {
            if (yCond) {
                bar.increaseScore();
                LookAndFeel.playSound(Const.SpaceDodge.soundpaths[1]);
            } else bar.loseHeart();
            newObstacle();
        }
