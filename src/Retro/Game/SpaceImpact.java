...
import Retro.Entity.Shot;

import java.awt.*;
import java.util.Timer;
import java.util.TimerTask;

public class SpaceImpact extends Game {
    ...
    public SpaceImpact(String name) {
        super(name);
        setup();
    }
    ...
    private void shoot() {
        for (int i = 0; i < maxShots; i++) {
            if (shots[i] == null) {
                shots[i] = new Shot(6, 3, Color.black);
                shots[i].setVelX(4);
    ...
    private void setup() {
        ...
        character = new Entity(charactersize, charactersize, Const.SpaceImpact.iconpaths[0]);
    ...
    private void newEnemy() {
        for (int i = 0; i < maxEnemies; i++) {
            if (enemies[i] == null) {
                int enemyHeight = 25;
                int enemyWidth = 20;
                enemies[i] = new Entity(enemyWidth, enemyHeight, Const.SpaceImpact.iconpaths[1]);
                enemies[i].setVelX(-5);
